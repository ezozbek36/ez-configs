...
let
  users = nixosConfig.users.users;
in
{
  imports = [ ezModules.direnv ];

  home = rec {
    username = if (users ? system-user) then users.system-user.name else "example-user";
    stateVersion = "26.05";
    homeDirectory =
      if (users ? system-user) then
        users.system-user.home
      else
